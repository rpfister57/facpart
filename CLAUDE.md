...
The package site is deployed to `https://rpfister57.github.io/facpart/` via the workflow at `.github/workflows/pkgdown.yaml`. It triggers on push to `main`, on published releases, and manually via `workflow_dispatch`.

The workflow runs `pkgdown::build_site_github_pages()` (output goes to `docs/`) and then deploys `docs/` to the `gh-pages` branch using `JamesIves/github-pages-deploy-action`. Configuration is in `_pkgdown.yml` (Bootstrap 5 template; URL matches `DESCRIPTION`'s `URL:` field).
