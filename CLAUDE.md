...
After editing `vignettes/FacetedPartitions.Rmd`, the pre-built copy in
`inst/doc/` must be updated and committed. `devtools::build_vignettes()`
writes output to the root-level `doc/` (a local-install artifact); it does
**not** update `inst/doc/`. The correct steps are:

```r
devtools::build_vignettes()          # rebuilds; output lands in doc/
file.copy(                           # sync to inst/doc/
  list.files("doc", full.names = TRUE),
  "inst/doc/", overwrite = TRUE)
devtools::install()                  # reinstall so help page picks it up
```

Then commit the changes to both `vignettes/FacetedPartitions.Rmd` and
`inst/doc/`. Never add `inst/doc` to `.gitignore` — its contents must be
committed so the vignette ships with the package regardless of how it is
installed.
